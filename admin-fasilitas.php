...
<body class="bg-gray-100 font-sans antialiased text-gray-800 flex h-screen overflow-hidden">
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden relative">
        <header class="h-16 bg-white shadow-sm flex items-center justify-between px-6 z-10">
            ...
            <a href="asrama.html" target="_blank" class="text-sm text-emerald-600 hover:text-emerald-800 font-medium"><i
                    class="fas fa-external-link-alt mr-1"></i> Lihat Web</a>
        </header>

        <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-50 p-6">
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-gray-900"><i
                        class="fas fa-building text-blue-600 mr-2"></i>Pengaturan Fasilitas Sekolah</h1>
            </div>
            <?php if (isset($pesan_sukses))
                echo "<div class='bg-green-100 text-green-700 px-4 py-3 rounded-lg mb-6 shadow-sm'><i class='fas fa-check-circle mr-2'></i> $pesan_sukses</div>"; ?>

            <!-- FORM INPUT (2 KOLOM) -->
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-8">
                <div class="px-6 py-4 bg-blue-50 border-b border-blue-100">
                    <h2 class="font-bold text-blue-800"><i
                            class="fas <?= $edit_mode ? 'fa-edit' : 'fa-plus' ?> mr-2"></i><?= $edit_mode ? 'Edit Fasilitas' : 'Tambah Fasilitas Baru' ?>
                    </h2>
                </div>
                <form action="admin-fasilitas.php" method="POST" class="p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-1">URL Gambar <span
                                        class="text-red-500">*</span></label>
                                <input type="text" name="gambar_url" id="img-url"
                                    value="<?= $edit_mode ? htmlspecialchars($data_edit['gambar_url']) : '' ?>" required
                                    class="w-full px-4 py-2 border rounded-lg focus:ring-blue-500 focus:border-blue-500"
                                    placeholder="https://..."
                                    oninput="document.getElementById('img-preview').src=this.value">
                            </div>
                            <div
                                class="bg-gray-100 p-2 rounded-lg border border-gray-200 h-48 flex items-center justify-center overflow-hidden">
                                <img id="img-preview"
                                    src="<?= $edit_mode ? htmlspecialchars($data_edit['gambar_url']) : '' ?>"
                                    class="max-h-full object-contain"
                                    onerror="this.src='https://via.placeholder.com/600x400?text=Preview+Gambar'"
                                    alt="Preview">
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-1">Judul Fasilitas <span
                                        class="text-red-500">*</span></label>
                                <input type="text" name="judul"
                                    value="<?= $edit_mode ? htmlspecialchars($data_edit['judul']) : '' ?>" required
                                    class="w-full px-4 py-2 border rounded-lg focus:ring-blue-500 focus:border-blue-500"
                                    placeholder="Contoh: Masjid Jami' Pesantren">
                            </div>
                            <div>
                                ...
                                <textarea name="deskripsi" rows="5" required
                                    class="w-full px-4 py-2 border rounded-lg focus:ring-blue-500 focus:border-blue-500"
                                    placeholder="Jelaskan kegunaan dan keunggulan fasilitas ini..."><?= $edit_mode ? htmlspecialchars($data_edit['deskripsi']) : '' ?></textarea>
                            </div>
                            <div class="pt-4 text-right">
                                <?php if ($edit_mode)
                                    echo '<a href="admin-fasilitas.php" class="bg-gray-200 text-gray-700 px-6 py-2 rounded-lg font-bold mr-2">Batal</a>'; ?>
                                <button type="submit"
                                    class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-6 rounded-lg shadow-md transition"><i
                                        class="fas fa-save mr-1"></i>
                                    <?= $edit_mode ? 'Update' : 'Simpan Fasilitas' ?></button>
                            </div>
                        </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-6 py-4 bg-gray-50 border-b border-gray-100">
                    <h2 class="font-bold text-gray-800">Daftar Fasilitas Tersedia</h2>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-white">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase">Keterangan
                                </th>
                                <th class="px-4 py-3 text-center text-xs font-bold text-gray-500 uppercase w-32">Aksi
                                </th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            ...
                            if ($res && $res->num_rows > 0) {
                                while ($row = $res->fetch_assoc()) { ?>
                                    <tr>
                                        <td class="px-4 py-3"><img src="<?= htmlspecialchars($row['gambar_url']) ?>"
                                                class="w-24 h-16 rounded object-cover shadow-sm"></td>
                                        <td class="px-4 py-3">
                                            <div class="font-bold text-gray-900"><?= htmlspecialchars($row['judul']) ?></div>
                                            <div class="text-sm text-gray-500 mt-1"><?= htmlspecialchars($row['deskripsi']) ?>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3 text-center font-medium">
                                            <a href="?edit_id=<?= $row['id'] ?>" class="text-blue-600 hover:text-blue-900 mr-3"
                                                title="Edit"><i class="fas fa-edit"></i></a>
                                            <a href="?hapus_id=<?= $row['id'] ?>"
                                                onclick="return confirm('Yakin menghapus data fasilitas ini?')"
                                                class="text-rose-600 hover:text-rose-900" title="Hapus"><i
                                                    class="fas fa-trash"></i></a>
                                        </td>
                                    </tr>
                                <?php }
                            } else {
                                echo "<tr><td colspan='3' class='text-center py-6 text-gray-500'>Belum ada data fasilitas.</td></tr>";
                            } ?>
                        </tbody>
                    </table>
                </div>
