...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($target_ustadz_id > 0) {
        ...
        $sql = "INSERT INTO kesediaan_mengajar (ustadz_id, mapel_1_id, mapel_2_id, mapel_3_id, hari_1, hari_2, hari_3, jam_1, jam_2, jam_3) 
                VALUES ($target_ustadz_id, $mapel_1_id, $mapel_2_id, $mapel_3_id, '$hari_1', '$hari_2', '$hari_3', '$jam_1', '$jam_2', '$jam_3')
                ON DUPLICATE KEY UPDATE
                mapel_1_id = VALUES(mapel_1_id), mapel_2_id = VALUES(mapel_2_id), mapel_3_id = VALUES(mapel_3_id),
...
$mapel_list = [];
$res_mapel = $conn->query("SELECT id, nama_mapel, kategori_mapel FROM master_mapel WHERE kategori_mapel IN ('Diknas', 'Diniyah') ORDER BY kategori_mapel, nama_mapel ASC");
if ($res_mapel) {
                ...
                <form action="admin-pegawai-kesediaan.php<?= $ustadz_to_load > 0 ? '?ustadz_id='.$ustadz_to_load : '' ?>" method="POST" class="p-6">
                    ...
                    <div class="border-t pt-6 mt-6">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4">
                            <div class="font-bold text-gray-700">Mata Pelajaran</div><div class="font-bold text-gray-700">Pilihan Hari</div><div class="font-bold text-gray-700">Pilihan Jam</div>
                            <?php for ($i = 1; $i <= 3; $i++): ?>
                                ...
                                <div><input type="text" name="jam_<?= $i ?>" value="<?= htmlspecialchars($data_kesediaan['jam_'.$i] ?? '') ?>" class="w-full px-3 py-2 border rounded-lg text-sm" placeholder="Cth: 07:00 - 08:30"></div>
                            <?php endfor; ?>
                        </div>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <?php else: foreach($semua_kesediaan as $d): ?>
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3 text-sm"><div><b><?= htmlspecialchars($d['mapel1'] ?? '-') ?></b></div><div class="text-gray-600"><?= htmlspecialchars($d['hari_1'] ?? '-') ?>, <?= htmlspecialchars($d['jam_1'] ?? '-') ?></div></td>
                                    <td class="px-4 py-3 text-sm"><div><b><?= htmlspecialchars($d['mapel2'] ?? '-') ?></b></div><div class="text-gray-600"><?= htmlspecialchars($d['hari_2'] ?? '-') ?>, <?= htmlspecialchars($d['jam_2'] ?? '-') ?></div></td>
                                    <td class="px-4 py-3 text-sm"><div><b><?= htmlspecialchars($d['mapel3'] ?? '-') ?></b></div><div class="text-gray-600"><?= htmlspecialchars($d['hari_3'] ?? '-') ?>, <?= htmlspecialchars($d['jam_3'] ?? '-') ?></div></td>
                                </tr>
