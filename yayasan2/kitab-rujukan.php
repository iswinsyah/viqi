...
if (isset($_GET['hapus_id'])) {
    ...
    if ($conn->query("DELETE FROM master_kitab WHERE id = $id")) {
        header("Location: kitab-rujukan.php?sukses=" . urlencode("Kitab rujukan berhasil dihapus!"));
    } else {
        header("Location: kitab-rujukan.php?error=" . urlencode("Gagal menghapus data: " . $conn->error));
    }
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($conn->query($sql)) {
        header("Location: kitab-rujukan.php?sukses=" . urlencode($pesan_sukses));
        exit;
    } else {
...
<head>
    ...
    <title>Daftar Kitab Rujukan | Panel Yayasan</title>
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <header class="h-16 bg-white shadow-sm flex items-center justify-between px-6 z-10 flex-shrink-0">
            <div class="flex items-center">
                <button id="open-sidebar-yayasan2" class="text-gray-500 md:hidden mr-4">
                    <i class="fas fa-bars text-xl"></i>
                </button>
                <h2 class="font-bold text-gray-800 hidden sm:block">Panel Eksekutif Yayasan</h2>
            </div>
            <div class="flex items-center space-x-3">
                <span class="text-xs font-semibold bg-amber-100 text-amber-800 px-3 py-1 rounded-full"><i class="fas fa-user-tie mr-1"></i> Admin Yayasan</span>
            </div>
        </header>
        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto bg-slate-50/50 p-6">
            <!-- TITLE BAR -->
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-gray-900"><i class="fas fa-book-open text-amber-600 mr-2.5"></i>Daftar Kitab Rujukan</h1>
            ...
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200/80 overflow-hidden flex flex-col h-fit">
                    ...
                    <form action="kitab-rujukan.php" method="POST" class="p-6 space-y-5">
                        ...
                        <div>
                            ...
                            <input type="text" name="nama_kitab" value="<?= $edit_mode ? htmlspecialchars($data_edit['nama_kitab']) : '' ?>" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 text-sm" placeholder="Contoh: Tafsir Jalalain">
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1.5">Pengarang / Penulis</label>
                            <input type="text" name="pengarang" value="<?= $edit_mode ? htmlspecialchars($data_edit['pengarang']) : '' ?>" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 text-sm" placeholder="Contoh: Jalaluddin al-Mahalli & Jalaluddin as-Suyuthi">
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1.5">Penerbit</label>
                            <input type="text" name="penerbit" value="<?= $edit_mode ? htmlspecialchars($data_edit['penerbit']) : '' ?>" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 text-sm" placeholder="Contoh: Darul Hadits Kairo">
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1.5">Harga Beli / Satuan (Rp)</label>
                            <input type="number" step="1" name="harga" value="<?= $edit_mode ? htmlspecialchars($data_edit['harga']) : '0' ?>" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 text-sm" placeholder="Contoh: 125000">
                        </div>

                        <div class="text-right pt-2">
                            <?php if ($edit_mode): ?>
                                <a href="kitab-rujukan.php" class="inline-block bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-2 px-4 rounded-lg text-sm transition-colors mr-1">Batal</a>
                            <?php endif; ?>
                            <button type="submit" class="bg-amber-600 hover:bg-amber-700 text-white font-bold py-2 px-6 rounded-lg text-sm shadow transition-colors">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200/80 overflow-hidden xl:col-span-2 flex flex-col">
                    <div class="px-6 py-4 bg-slate-50 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <h2 class="font-bold text-gray-800 text-sm flex items-center">
                            <i class="fas fa-list mr-2 text-amber-600"></i>Daftar Kitab Terdaftar
                        </h2>
                        <!-- SEARCH BAR -->
                        <div class="relative w-full sm:w-64">
                            ...
                            <input type="text" id="searchKitab" onkeyup="filterKitabTable()" class="w-full pl-9 pr-4 py-1.5 bg-white border border-gray-300 rounded-lg text-xs focus:ring-1 focus:ring-amber-500 focus:border-amber-500" placeholder="Cari nama kitab, pengarang...">
                        </div>
                    </div>
                                ...
                                ?>
                                        <tr class="hover:bg-slate-50/80 transition-colors">
                                            <td class="px-5 py-3.5 font-bold text-gray-800">
                                                <i class="fas fa-book text-amber-700/70 mr-1.5"></i> <?= htmlspecialchars($row['nama_kitab']) ?>
                                            </td>
                                            ...
                                            <td class="px-5 py-3.5 text-center">
                                                <div class="flex items-center justify-center space-x-1.5">
                                                    <a href="kitab-rujukan.php?edit_id=<?= $row['id'] ?>" class="text-amber-600 hover:text-amber-800 bg-amber-50 hover:bg-amber-100 p-1.5 rounded transition text-xs" title="Edit Kitab">
    ...
    <script>
        ...
        function confirmDelete(id, name) {
            if (confirm(`Apakah Anda yakin ingin menghapus kitab rujukan "${name}"?`)) {
                window.location.href = `kitab-rujukan.php?hapus_id=${id}`;
            }
        }
