...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_status'])) {
    ...
    $sql = "UPDATE uang_saku SET status = '$status', catatan_admin = '$catatan' WHERE id = $id";
    if ($conn->query($sql)) {
        $pesan_sukses = "Validasi uang saku berhasil diperbarui!";
    } else {
        $pesan_error = "Gagal memperbarui status: " . $conn->error;
    }
}

// 2. Ambil Seluruh Data
...
<head>
    ...
    <title>Rekap Uang Saku | Panel Yayasan</title>
...
<body class="bg-gray-100 font-sans antialiased text-gray-800 flex h-screen overflow-hidden">
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden relative">
        <header class="h-16 bg-white shadow-sm flex items-center justify-between px-6 z-10 flex-shrink-0">
            <div class="flex items-center"><button id="open-sidebar-yayasan2" class="text-gray-500 hover:text-gray-700 md:hidden mr-4"><i class="fas fa-bars text-xl"></i></button><h2 class="font-bold text-gray-800">Rekap Kirim Uang Saku</h2></div>
        </header>
        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-50 p-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-100 bg-amber-50"><h2 class="font-bold text-amber-800">Semua Data Konfirmasi Uang Saku</h2></div>
                <div class="overflow-x-auto p-4">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="divide-y divide-gray-100 text-sm">
                            <?php if(empty($data_us)): ?><tr><td colspan="6" class="text-center py-10 text-gray-400 italic">Belum ada data uang saku masuk.</td></tr><?php else: foreach($data_us as $r): 
                            ...
                            ?>
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3 text-center"><?php if($r['bukti_transfer']): ?><a href="../uploads/uang_saku/<?= $r['bukti_transfer'] ?>" target="_blank" class="text-amber-600 hover:text-amber-800"><i class="fas fa-image text-lg"></i></a><?php else: ?>-<?php endif; ?></td>
