...
$active_menu = 'rapot_pkbm_santri';

// Fetch santri data
$res_s = $conn->query("SELECT * FROM buku_induk_santri WHERE id = $santri_id");
$data_santri = ($res_s) ? $res_s->fetch_assoc() : null;

// Determine Paket B vs Paket C based on class
...
<head>
    ...
    <title>Raport Diknas PKBM <?= $paket_tipe ?> | Ruang Santri</title>
...
<body class="bg-indigo-50/30 font-sans antialiased text-slate-800 flex h-screen overflow-hidden">
            ...
            <div class="flex items-center">
                ...
                <h2 class="font-bold text-slate-800 text-base sm:text-lg flex items-center gap-2">
                    <span class="bg-indigo-100 text-indigo-800 px-2.5 py-1 rounded-lg text-xs font-extrabold uppercase tracking-wide">
                        <i class="fas fa-graduation-cap mr-1"></i>Ruang Santri
                    </span>
                    <span>SADIGS 4.0</span>
                </h2>
            </div>
            <div class="flex items-center space-x-3">
                <button onclick="window.print()" class="bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold px-4 py-2 rounded-xl shadow-md transition flex items-center gap-2">
            ...
            <?php if ($data_santri): ?>
                <div class="print-area bg-white p-8 sm:p-12 rounded-2xl border border-slate-200/90 shadow-lg max-w-4xl mx-auto text-slate-900 text-xs sm:text-sm leading-relaxed">
                    ...
                    <div class="text-center border-b-2 border-slate-900 pb-4 mb-6">
                        ...
                        <h1 class="text-xl sm:text-2xl font-black uppercase text-indigo-800 tracking-tight my-0.5">PKBM VILLA QUR'AN INDONESIA</h1>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-6 bg-slate-50/80 p-4 rounded-xl border border-slate-200 text-xs">
                        <div class="space-y-1.5">
                            <div class="flex"><span class="w-32 font-bold text-slate-600">Nama Santri:</span> <span class="font-black text-slate-900 uppercase"><?= htmlspecialchars($data_santri['nama_lengkap']) ?></span></div>
                        ...
                        <div class="space-y-1.5">
                            <div class="flex"><span class="w-32 font-bold text-slate-600">Kelas / Tingkat:</span> <span class="font-bold text-indigo-800"><?= htmlspecialchars($data_santri['kelas_sekarang']) ?> (<?= $paket_tipe ?>)</span></div>
                    ...
                    <div class="mb-6">
                        <h3 class="font-extrabold text-sm uppercase text-slate-900 mb-3 border-b-2 border-indigo-600 pb-1">A. Capaian Hasil Belajar (Nilai Akademik Diknas)</h3>
