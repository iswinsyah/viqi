...
$user_roles_str = $_SESSION['ustadz_role'] ?? '';
$user_roles = !empty($user_roles_str) ? explode(',', $user_roles_str) : [];

// Fungsi helper untuk memeriksa peran
function has_role($needed_roles, $user_roles) {
    if (in_array('super_admin', $user_roles)) return true; // Super admin bisa lihat semua
...
<aside id="sidebar-hr" class="bg-slate-800 text-white w-64 flex-shrink-0 hidden md:flex flex-col z-30 transition-all duration-300 absolute md:relative h-full shadow-2xl">
    <div class="h-16 flex items-center justify-between px-6 border-b border-slate-700 bg-slate-900">
        <h1 class="font-extrabold text-lg tracking-wider flex items-center text-cyan-400">
            <i class="fas fa-users-cog mr-2"></i> RUANG STAF
        </h1>
    ...
    <div class="flex-1 overflow-y-auto py-4">
        <nav class="px-4 space-y-1">
            ...
            <a href="admin-ustadz.php" class="<?= (isset($active_menu) && $active_menu == 'dashboard_pegawai') ? 'bg-slate-700 text-white' : 'text-slate-100 hover:bg-slate-700 hover:text-white' ?> group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-all">
                <i class="fas fa-tachometer-alt w-6 text-center mr-2 <?= (isset($active_menu) && $active_menu == 'dashboard_pegawai') ? 'text-cyan-400' : 'text-slate-300 group-hover:text-white' ?>"></i> Dashboard Staf
            </a>
            
            <?php if (has_role(['ustadz', 'kepala_sekolah', 'sekretaris_sekolah', 'admin_sekolah'], $user_roles)): ?>
            <p class="px-2 text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 mt-6">Menu Asatidz</p>
            ...
            <?php if (has_role(['musyrif', 'kepala_asrama', 'kepala_sekolah'], $user_roles)): ?>
            <p class="px-2 text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 mt-6">Menu Musyrif</p>
            ...
            <p class="px-2 text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 mt-6">Menu Staf</p>
            ...
            <p class="px-2 text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 mt-6">Menu KPI</p>
            ...
            <p class="px-2 text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 mt-6">Menu Salary</p>
            <!-- Placeholder untuk menu salary mendatang -->
        </nav>
    </div>
