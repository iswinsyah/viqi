...
// Check role: kepala_asrama, kepala_asrama_rijal, super_admin, kepala_mahad
$user_roles = [];
...
foreach ($user_roles as $role) {
    ...
    if (in_array($norm_role, ['super_admin', 'kepala_asrama', 'kepala_asrama_rijal', 'kepala_mahad', 'musyrif'])) {
        $is_authorized = true;
        break;
    }
}
...
// Fetch Rijal Ibadah Records (Laki-laki)
$query = "
...
$res_stat1 = $conn->query("
    ...
    FROM ibadah_harian_santri i 
    JOIN buku_induk_santri s ON i.santri_id = s.id 
    WHERE (s.jenis_kelamin = 'Laki-laki' OR s.jenis_kelamin IS NULL) AND DATE(i.created_at) = CURDATE()
");
...
$active_menu = 'rekap_ibadah_rijal';
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Rekap Ibadah Santri Rijal (Putra) | Ruang Asatidz</title>
        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto bg-slate-50 p-4 sm:p-6 lg:p-8">
            ...
            <div class="mb-6">
                <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight flex items-center gap-3">
                    ...
                    <span>Tembusan Ibadah Harian Santri Rijal (Putra)</span>
                </h1>
                <p class="text-xs sm:text-sm text-slate-500 mt-1">Rekapan ibadah harian santri putra yang divalidasi oleh Musyrif untuk Kepala Asrama Rijal</p>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div class="text-xs font-semibold text-slate-500">Laporan Rijal Masuk Hari Ini</div>
                    </div>
                </div>
                ...
                <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div class="text-xs font-semibold text-slate-500">Laporan Rijal Divalidasi Musyrif</div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
                <div class="px-6 py-4 bg-slate-50 border-b border-slate-200/80 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <h2 class="font-bold text-slate-800 text-base flex items-center gap-2">
                        ...
                        <span>Daftar Ibadah Santri Putra</span>
                    </h2>
                    ...
                    <table class="w-full text-left border-collapse" id="ibadah_table">
                        <thead>
                            <tr class="bg-slate-100/70 border-b border-slate-200 text-slate-600 uppercase text-[11px] font-bold tracking-wider">
                                <th class="py-3.5 px-6">Santri Rijal & Tanggal</th>
                        ...
                        <tbody class="divide-y divide-slate-100 text-xs sm:text-sm">
                            <?php if ($res_data && $res_data->num_rows > 0): ?>
                                    ...
                                    <tr class="hover:bg-slate-50 transition">
                                        <td class="py-3.5 px-6 align-top">
                                            ...
                                            <div class="text-[11px] text-slate-500 mt-1">
                                                <i class="far fa-calendar-alt text-blue-600 mr-1"></i><b><?= date('d M Y', strtotime($row['tanggal'])) ?></b>
                                            </div>
                                        </td>
                                ...
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr><td colspan="4" class="py-8 text-center text-slate-400 italic">Belum ada data ibadah harian santri Rijal.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
