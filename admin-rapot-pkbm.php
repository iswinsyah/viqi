...
// Fetch Grades for Selected Santri
$nilai_mapel = [];
if ($selected_santri) {
...
if ($selected_santri) {
    $s_id = (int)$selected_santri['id'];
    $k_esc = $conn->real_escape_string($selected_santri['kelas_sekarang']);
    $ta_esc = $conn->real_escape_string($filters['tahun_ajaran']);
    $sem_esc = $conn->real_escape_string($filters['semester']);
        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto bg-slate-100 p-4 sm:p-6 lg:p-8">
            ...
            <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm mb-6 no-print">
                <form method="GET" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4 items-end">
                    ...
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1.5">Santri</label>
                        <select name="santri_id" onchange="this.form.submit()" class="w-full px-3 py-2 border border-slate-300 rounded-xl text-xs bg-white font-semibold">
