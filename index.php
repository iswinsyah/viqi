...
// Mengamankan struktur tabel gaji (Bypass jika error)
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_gaji'])) {
    ...
    $pesan_sukses = "Pengaturan Tarif Gaji & Bonus (Per Pertemuan) berhasil diperbarui!";
}
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-50 p-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
                <h2 class="font-bold text-gray-800 mb-4 border-b pb-2"><i class="fas fa-money-bill-wave text-indigo-500 mr-2"></i>Pengaturan Gaji & Bonus (Per Pertemuan)</h2>
                <form action="" method="POST" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    <input type="hidden" name="update_gaji" value="1">
                    <div>
                        <label class="block text-xs font-bold text-gray-700 mb-1">Gaji Per Pertemuan Grade C (Rp)</label>
                        <input type="number" name="gaji_grade_c" value="<?= $gaji_grade_c ?>" class="w-full px-4 py-2 border rounded-lg focus:ring-indigo-500" placeholder="Cth: 20000" required>
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-700 mb-1">Gaji Per Pertemuan Grade B (Rp)</label>
                        <input type="number" name="gaji_grade_b" value="<?= $gaji_grade_b ?>" class="w-full px-4 py-2 border rounded-lg focus:ring-indigo-500" placeholder="Cth: 22500" required>
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-700 mb-1">Gaji Per Pertemuan Grade A (Rp)</label>
                        <input type="number" name="gaji_grade_a" value="<?= $gaji_grade_a ?>" class="w-full px-4 py-2 border rounded-lg focus:ring-indigo-500" placeholder="Cth: 25000" required>
                    </div>
                    <div>
