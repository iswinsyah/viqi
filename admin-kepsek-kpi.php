...
if (!$is_super_admin && !$is_current_kepsek) {
    die("Akses ditolak. Halaman ini hanya dapat diakses oleh Kepala Sekolah atau Super Admin.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_laporan_kepsek'])) {
    ...
    // Ensure users can only edit their own report unless they are Admin
...
// --- FETCH GRADE CONFIGURATION FOR REFERENCE ---
...
<head>
    ...
    <title>KPI Kepala Sekolah | Portal Asatidz</title>
        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto bg-slate-50 p-6 text-left">
            <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-1">Laporan kinerja bulanan Kepala Sekolah berdasarkan target manajerial, supervisi KBM, kontrol RPP, kehadiran rapat, dan standarisasi nilai KKM.</p>
                </div>
                <!-- Month Filter -->
                <form method="GET" class="flex flex-wrap items-center gap-2">
                    <?php if (!$is_current_kepsek && !empty($kepsek_list)): ?>
                        <select name="kepsek_id" onchange="this.form.submit()" class="px-4 py-2 bg-white border border-slate-350 rounded-xl text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                            <?php foreach ($kepsek_list as $k): ?>
                                <option value="<?= $k['id'] ?>" <?= $k['id'] == $selected_kepsek_id ? 'selected' : '' ?>><?= htmlspecialchars($k['nama']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    <?php endif; ?>
                    <input type="month" name="periode" value="<?= $selected_period ?>" onchange="this.form.submit()" class="px-4 py-2 bg-white border border-slate-350 rounded-xl text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                </form>
            </div>
            ...
            <?php if ($selected_kepsek_id <= 0): ?>
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 text-center text-slate-500 italic text-sm">
                    <i class="fas fa-info-circle text-emerald-500 text-2xl mb-2"></i><br>
                    Belum ada guru dengan peran Kepala Sekolah terdaftar di sistem.
                </div>
            <?php else: ?>
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                    <!-- KPI Summary Card -->
                    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col items-center justify-center text-center lg:col-span-1">
                        <span class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Total Nilai KPI</span>
                        <div class="text-5xl font-black text-emerald-600 mb-2"><?= number_format($total_kpi, 1) ?><span class="text-lg text-slate-400">/100</span></div>
                        <span class="px-3 py-1 text-xs font-bold rounded-full border <?= $color_predikat ?> mb-4"><?= $predikat ?></span>
                        <p class="text-[10px] text-slate-400 font-medium">Periode: <b><?= date('F Y', strtotime($selected_period . '-01')) ?></b><br>Kepala Sekolah: <b><?= htmlspecialchars($selected_kepsek_name) ?></b></p>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 lg:col-span-2">
                        ...
                        <form method="POST" class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-xs font-bold text-slate-600 mb-1">Total Jam KBM Formal Terjadwal (Bulan Ini)</label>
                                    <input type="number" name="total_kbm_terjadwal" value="<?= $kbm_terjadwal ?>" class="w-full px-3 py-2 border border-slate-200 rounded-xl text-xs focus:ring-2 focus:ring-emerald-500" required placeholder="Contoh: 120">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-slate-600 mb-1">Total Jam KBM Terlaksana (Bulan Ini)</label>
                                    <input type="number" name="total_kbm_terlaksana" value="<?= $kbm_terlaksana ?>" class="w-full px-3 py-2 border border-slate-200 rounded-xl text-xs focus:ring-2 focus:ring-emerald-500" required placeholder="Contoh: 114">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-slate-600 mb-1">Total RPP Pelajaran Diknas Wajib (Bulan Ini)</label>
                                    <input type="number" name="rpp_diknas_total" value="<?= $rpp_total ?>" class="w-full px-3 py-2 border border-slate-200 rounded-xl text-xs focus:ring-2 focus:ring-emerald-500" required placeholder="Contoh: 15">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-slate-600 mb-1">Total RPP yang Dikontrol & Disetujui (Bulan Ini)</label>
                                    <input type="number" name="rpp_diknas_dikontrol" value="<?= $rpp_dikontrol ?>" class="w-full px-3 py-2 border border-slate-200 rounded-xl text-xs focus:ring-2 focus:ring-emerald-500" required placeholder="Contoh: 15">
                                </div>
                            </div>
                ...
                <div class="space-y-4">
                    <h2 class="font-bold text-slate-800 text-base mb-2"><i class="fas fa-list-check text-emerald-600 mr-1.5"></i>Rincian 5 Aspek Penilaian KPI</h2>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-slate-500 mt-1">Mengontrol pelaksanaan KBM harian formal dengan target keaktifan minimal **90%**. Jika keaktifan $\ge 90\%$, mendapat poin penuh.</p>
                            <!-- Progress Bar -->
                            ...
                            <span class="text-[10px] text-slate-400 block mt-1">Kehadiran KBM: <b><?= $kbm_terlaksana ?></b> dari <b><?= $kbm_terjadwal ?></b> jam terjadwal (<b><?= number_format($persen_kbm, 1) ?>%</b>)</span>
                        </div>
                        <div class="text-right flex flex-col justify-center min-w-[80px]">
                            <span class="text-2xl font-bold text-slate-800"><?= number_format($score_kbm, 1) ?></span>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-slate-500 mt-1">Melakukan supervisi pengajaran ustadz/ustadzah formal secara langsung. Target minimal **2 kali supervisi per bulan**.</p>
                            <!-- Progress Bar -->
                            <div class="w-full bg-slate-100 h-2.5 rounded-full mt-3 overflow-hidden">
                                <div class="bg-blue-600 h-2.5 rounded-full" style="width: <?= min(100, $score_supervisi) ?>%"></div>
                            </div>
                            <span class="text-[10px] text-slate-400 block mt-1">Supervisi dilakukan: <b><?= $total_supervisi ?></b> kali dari target <b>2</b> kali sebulan (data terhitung otomatis)</span>
                        </div>
                        <div class="text-right flex flex-col justify-center min-w-[80px]">
                            <span class="text-2xl font-bold text-slate-800"><?= number_format($score_supervisi, 1) ?></span>
                            <span class="text-[10px] text-slate-400 uppercase tracking-wider block">Skor Aspek</span>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-slate-500 mt-1">Mengontrol dan menyetujui pengadaan RPP (Rencana Pelaksanaan Pembelajaran) guru untuk mapel diknas. Target **100%**.</p>
                            <!-- Progress Bar -->
                            <div class="w-full bg-slate-100 h-2.5 rounded-full mt-3 overflow-hidden">
                                <div class="bg-purple-600 h-2.5 rounded-full" style="width: <?= min(100, $score_rpp) ?>%"></div>
                            </div>
                            <span class="text-[10px] text-slate-400 block mt-1">RPP Dikontrol: <b><?= $rpp_dikontrol ?></b> dari <b><?= $rpp_total ?></b> wajib dikontrol</span>
                        </div>
                        <div class="text-right flex flex-col justify-center min-w-[80px]">
                            <span class="text-2xl font-bold text-slate-800"><?= number_format($score_rpp, 1) ?></span>
                            <span class="text-[10px] text-slate-400 uppercase tracking-wider block">Skor Aspek</span>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-slate-500 mt-1">Menghadiri rapat koordinasi 2-pekanan bersama Yayasan/Pimpinan Ma'had. Target minimal **2 kali rapat per bulan**.</p>
                            <!-- Progress Bar -->
                            <div class="w-full bg-slate-100 h-2.5 rounded-full mt-3 overflow-hidden">
                                <div class="bg-emerald-650 h-2.5 rounded-full" style="width: <?= min(100, $score_rapat) ?>%"></div>
                            </div>
                            <span class="text-[10px] text-slate-400 block mt-1">Kehadiran Rapat: <b><?= $total_rapat_hadir ?></b> kali dari target <b>2</b> kali rapat (data terhitung otomatis)</span>
                        </div>
                        <div class="text-right flex flex-col justify-center min-w-[80px]">
                            <span class="text-2xl font-bold text-slate-800"><?= number_format($score_rapat, 1) ?></span>
                            <span class="text-[10px] text-slate-400 uppercase tracking-wider block">Skor Aspek</span>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-slate-500 mt-1">Memastikan nilai akademis rata-rata santri di atas standar KKM (75) pada tengah semester (Oktober, Maret) dan akhir semester (Juni, Desember).</p>
                            <!-- Progress Bar -->
                            <div class="w-full bg-slate-100 h-2.5 rounded-full mt-3 overflow-hidden">
                                <div class="bg-teal-600 h-2.5 rounded-full" style="width: <?= min(100, $score_nilai) ?>%"></div>
                            </div>
