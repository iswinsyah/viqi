...
// Allow musyrif, kepala_asrama, kepala_asrama_nisa, super_admin
$user_roles = [];
...
foreach ($user_roles as $role) {
    ...
    if (in_array($norm_role, ['super_admin', 'kepala_asrama', 'kepala_asrama_nisa', 'musyrif', 'kepala_mahad'])) {
        $is_authorized = true;
        break;
    }
}
if (!$is_authorized) {
    die('Akses ditolak. Halaman ini khusus untuk Kepala Asrama Nisa dan Pengurus Ma\'had.');
}

// Fetch Nisa Setoran Records (Perempuan)
$query = "
    SELECT l.*, s.jenis_kelamin, s.kelas_sekarang 
    FROM laporan_setoran_hafalan l 
    JOIN buku_induk_santri s ON (l.santri_id = s.id OR l.nama_santri = s.nama_lengkap) 
...
$res_stat2 = $conn->query("
    ...
    FROM laporan_setoran_hafalan l 
    JOIN buku_induk_santri s ON (l.santri_id = s.id OR l.nama_santri = s.nama_lengkap) 
    WHERE s.jenis_kelamin = 'Perempuan' AND DATE(l.created_at) = CURDATE()
");
...
$res_stat3 = $conn->query("
    ...
    FROM laporan_setoran_hafalan l 
    JOIN buku_induk_santri s ON (l.santri_id = s.id OR l.nama_santri = s.nama_lengkap) 
    WHERE s.jenis_kelamin = 'Perempuan' AND l.grade LIKE '%Mutqin%'
");
$total_mumtaz = ($res_stat3) ? $res_stat3->fetch_assoc()['total'] : 0;

$active_menu = 'laporan_setoran_nisa';
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Rekap Setoran Santri Nisa (Putri) | Ruang Asatidz</title>
        ...
        <header class="h-16 bg-white border-b border-slate-200 shadow-sm flex items-center justify-between px-6 z-10 flex-shrink-0">
            <div class="flex items-center">
                ...
                <h2 class="font-bold text-slate-800 text-base sm:text-lg flex items-center gap-2">
                    <span class="bg-pink-100 text-pink-800 px-2.5 py-1 rounded-lg text-xs font-extrabold uppercase tracking-wide">
                        <i class="fas fa-venus mr-1"></i>Kepala Asrama Nisa
                    </span>
            ...
            <div class="flex items-center space-x-3">
                <span class="hidden sm:inline-block text-xs font-semibold text-slate-500 bg-slate-100 px-3 py-1.5 rounded-full">
                    <i class="fas fa-calendar-day mr-1.5 text-pink-600"></i><?= date('d F Y') ?>
                </span>
            </div>
        </header>
        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto bg-slate-50 p-4 sm:p-6 lg:p-8">
            ...
            <div class="mb-6">
                <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl bg-gradient-to-tr from-pink-600 to-rose-500 flex items-center justify-center text-white shadow-md shadow-pink-200">
                        <i class="fas fa-female text-lg"></i>
                    </div>
                    <span>Tembusan Setoran Hafalan Santri Nisa (Putri)</span>
                </h1>
                <p class="text-xs sm:text-sm text-slate-500 mt-1">Rekapan otomatis laporan hafalan santri putri untuk pemantauan Kepala Asrama Nisa</p>
            </div>

            <!-- STATISTIC CARDS -->
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center gap-4">
                    <div class="w-12 h-12 rounded-xl bg-pink-50 text-pink-600 flex items-center justify-center text-xl font-bold">
                        <i class="fas fa-book-open"></i>
                    </div>
                    <div>
                        <div class="text-2xl font-black text-slate-800"><?= number_format($total_setoran) ?></div>
                        <div class="text-xs font-semibold text-slate-500">Total Setoran Nisa</div>
                    </div>
                </div>
                ...
                <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div class="text-xs font-semibold text-slate-500">Setoran Nisa Hari Ini</div>
                    </div>
                </div>
                ...
                <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div class="text-xs font-semibold text-slate-500">Capaian Mutqin Nisa</div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
                <div class="px-6 py-4 bg-slate-50 border-b border-slate-200/80 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <h2 class="font-bold text-slate-800 text-base flex items-center gap-2">
                        ...
                        <span>Daftar Setoran Santri Putri</span>
                    </h2>
                    <input type="text" id="search_input" onkeyup="filterTable()" placeholder="Cari santri Nisa..." 
                        class="px-3.5 py-1.5 border border-slate-300 rounded-lg text-xs focus:ring-2 focus:ring-pink-500 w-full sm:w-64">
                </div>
                    ...
                    <table class="w-full text-left border-collapse" id="setoran_table">
                        <thead>
                            <tr class="bg-slate-100/70 border-b border-slate-200 text-slate-600 uppercase text-[11px] font-bold tracking-wider">
                                <th class="py-3.5 px-6">Santri Nisa</th>
                        ...
                        <tbody class="divide-y divide-slate-100 text-xs sm:text-sm">
                            <?php if ($res_data && $res_data->num_rows > 0): ?>
                                ...
                                ?>
                                    <tr class="hover:bg-slate-50 transition">
                                        <td class="py-3.5 px-6 align-top">
                                            ...
                                            <span class="text-[10px] bg-pink-50 text-pink-700 px-2 py-0.5 rounded font-semibold mt-1 inline-block">Santri Nisa</span>
                                        </td>
                                        <td class="py-3.5 px-6 align-top">
                                            <div class="font-semibold text-pink-700 text-sm">
                                ...
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr><td colspan="5" class="py-8 text-center text-slate-400 italic">Belum ada rekapan setoran hafalan santri Nisa.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
